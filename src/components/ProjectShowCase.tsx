...
export default function ProjectShowCase(props: Readonly<Props>) {
  const { imageSrc, stack, title, date, type, description } = props;
      ...
      <div className="mt-4 rounded-md border-2 border-amber-600 p-6  dark:border-amber-100">
        <div className="flex justify-between">
          <p>
            ...
            <span className="font-semibold italic"> {stack.join(", ")}</span>
          </p>
          <p>
          ...
          <p>
            ...
            <span className="font-semibold italic"> {type}</span>
          </p>
        </div>
        {description && <p className="mt-4 font-bold">{description}</p>}
        {title === "KONAN" && (
          <p className="mx-auto mt-4 font-bold">
            More about{" "}
            <span>
              <span className="mr-1">
                <Link
